...
export class AnkiDoneScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const titleText = `Deck ${this.ankiDoneData.deckName}`
    ...
    const bestAnkiSpeed = storage.bestAnkiSpeed.get(this.ankiDoneData.deckName, this.ankiDoneData.deckSize);

    let newRecord = false;
    if (this.ankiDoneData.mistakes == 0) {
      if (!bestAnkiSpeed || (bestAnkiSpeed && this.ankiDoneData.duration < bestAnkiSpeed)) {
        storage.bestAnkiSpeed.set(this.ankiDoneData.deckName, this.ankiDoneData.deckSize, this.ankiDoneData.duration);
        newRecord = true;
      }
    }
    if (newRecord) {
    ...
    const starFrame = whichStarFrame(this.ankiDoneData.duration, this.ankiDoneData.mistakes);
